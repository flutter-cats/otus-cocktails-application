...
class _Benchmark {
  ...
  void timeStart(dynamic id) {
    start(id);
  }

  int timeEnd(dynamic id) {
    final String benchId = id.toString();
    if (!_starts.containsKey(benchId)) {
      throw Exception('Not found timer for id = $id');
    }
    final int result = DateTime.now().microsecondsSinceEpoch - _starts[benchId]!;
    _starts.remove(benchId);
    return result;
  }
}
